...
QUALITY_REGEXES = {
    quality: re.compile(pattern, re.IGNORECASE)
    for quality, pattern in QUALITY_PATTERNS.items()
}

QUALITY_SPEC_PATTERNS = {
...
QUALITY_SPEC_REGEXES = {
    quality: re.compile(pattern, re.IGNORECASE)
    for quality, pattern in QUALITY_SPEC_PATTERNS.items()
}
...
LANGUAGE_REGEXES = {
    language: re.compile(pattern, re.IGNORECASE)
    for language, pattern in LANGUAGE_PATTERNS.items()
}


def detect_quality(torrent_name):
    for quality, pattern in QUALITY_REGEXES.items():
        if pattern.search(torrent_name):
            return quality
    return ""
...
def detect_quality_spec(torrent_name):
    qualities = []
    for quality, pattern in QUALITY_SPEC_REGEXES.items():
        if pattern.search(torrent_name):
...
def detect_languages(torrent_name):
    languages = []
    for language, pattern in LANGUAGE_REGEXES.items():
        if pattern.search(torrent_name):
            languages.append(language)

    if LANGUAGE_REGEXES["multi"].search(torrent_name):
...
def post_process_results(link, media, debrid_service, url, result=None):
    ...
    result['playback'] = url
    result['filename'] = result.get('metadata_filename') or getMetadata(link, media.type)
