...
class QualityExclusionFilter(BaseFilter):
    ...
    def filter(self, data):
        filtered_items = []
        excluded_qualities = [quality.upper() for quality in self.config['selectedQualityExclusion']]
        rips = "RIPS" in excluded_qualities
        cams = "CAM" in excluded_qualities

        for stream in data:
            if stream['quality'].upper() not in excluded_qualities:
                detection = stream['quality_spec']
    ...
    def can_filter(self):
        return self.config['selectedQualityExclusion'] is not None and len(self.config['selectedQualityExclusion']) > 0
